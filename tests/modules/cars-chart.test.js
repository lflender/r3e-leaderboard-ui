...
beforeAll(() => {
    ...
    loadBrowserScript('modules/charts/cars-chart.js');
});
...
describe('CarsChart.getStats', () => {
    it('returns one entry per unique car', () => {
        const stats = window.CarsChart.getStats(SAMPLE_DATA);
    ...
    it('returns medianPosition 0 when no valid positions', () => {
        ...
        const stats = window.CarsChart.getStats(data);
    ...
    it('returns empty array for empty data', () => {
        expect(window.CarsChart.getStats([])).toEqual([]);
    });
});

describe('CarsChart.generateHtml', () => {
    it('returns an HTML string', () => {
        const html = window.CarsChart.generateHtml(SAMPLE_DATA);
    ...
    it('marks entries column as sort-active when sortBy=entries', () => {
        const html = window.CarsChart.generateHtml(SAMPLE_DATA, 'entries', 'desc');
    ...
    it('marks median column as sort-active when sortBy=median', () => {
        const html = window.CarsChart.generateHtml(SAMPLE_DATA, 'median', 'asc');
        expect(html).toContain('sort-active');
    ...
    it('sorts by median ascending', () => {
        ...
        const html = window.CarsChart.generateHtml(data, 'median', 'asc');
    ...
    it('expands when isExpanded=true', () => {
        const html = window.CarsChart.generateHtml(SAMPLE_DATA, 'entries', 'desc', true);
        expect(html).toContain('aria-expanded="true"');
    });

    it('renders dash for zero median position', () => {
        const data = [{ Car: 'No Pos Car', Position: '0' }];
        const html = window.CarsChart.generateHtml(data);
    ...
    it('generates wrapper HTML even for empty data (empty tbody)', () => {
        ...
        const html = window.CarsChart.generateHtml([]);
